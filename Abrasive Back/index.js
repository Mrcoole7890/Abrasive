...
const USER_ATTEMPTED_TO_JOIN_TWICE = function (user) {
  return user.username + " attempted to log in twice!";
};
const USER_JOINED_STRING = function (user) {
  ...
  socket.on(NEW_USER, (newUser) => {
    ...
    if (players.length != 0) {
      for (i = 0; i < players.length; i++) {
        ...
        if (players[i].getPlayer().getUsername().username == newUser.username)
          playerFound = true;
      }
    }
    if (playerFound) {
